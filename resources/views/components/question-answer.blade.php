...
<div class="flex-1">
  ...
  <p class="mt-3 text-gray-900 font-semibold">
    ...
    {{ $pivot->answer_count ?? '回答内容は利用できません' }}</a>

  <p class="mt-1 text-gray-900 font-semibold">
    ☆質問出現頻度       
  <select id="priority" class="mx-5 text-lg text-gray-900" data-question_id="{{ $question_answer->stored_question_id }}">
      <option value="1" {{ isset($pivot) && $pivot->priority == 0 ? 'selected' : '' }}>ごっちゃんです...</option>
